...
(*pr_non_empty_arg (prlist_with_sep spc
                                (pr_in_hyp_as
                                   (pr.pr_dconstr env sigma) pr.pr_name)) inhyp
*)
(*  let pr_in_hyp_as0 prc pr_id = function
  ...
  let pr_in_hyp_as prc pr_id = function
    ...
    | Some cl ->
      prlist_with_sep spc
        (fun (id, ipat) -> pr_in (spc () ++ pr_id id) ++ pr_as_ipat prc ipat)
        cl
      ...
      and pr_atom1 a = tag_atom a (match a with
        ...
        | TacApply (a,ev,cb,inhyp) ->
          hov 1 (
            ...
            pr_non_empty_arg
              (pr_in_hyp_as (pr.pr_dconstr env sigma) pr.pr_name)
              inhyp
          )
        | TacElim (ev,cb,cbo) ->
          hov 1 (
